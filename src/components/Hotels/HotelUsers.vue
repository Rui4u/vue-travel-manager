<template>
  <div>
    ...
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>{{hotelName}}</el-breadcrumb-item>
...
export default {
  ...
  created() {
    this.quertInfo.hotelId = this.$route.query.hotelId
  ...
  methods: {
    ...
    async getUserList() {
      const { data: res } = await this.$http.get('/mg/allUserByHotel', {
    ...
    async removeUserFromHotel(userId) {
      ...
      const { data: res } = await this.$http.post('/mg/userhoteldelete', {
        userId: userId,
        hotelId: this.quertInfo.hotelId
      })
      if (res.errno !== '0') {
        return this.$message.error(res.errmsg)
      }
