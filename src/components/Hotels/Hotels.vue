  ...
  <div>
    ...
    <el-card class="box-card">
      ...
      <el-table :data="allHotelList" border stripe>
        ...
        <el-table-column label="酒店附近信息">
          <template slot-scope="scope">
            <div class="hotelNearInfoDiv" v-for="(item, index) in scope.row.nearSite" :key="index">
              <label>{{item.siteName}}</label>
              <label>{{'距离/时间 :' + item.siteDistance + '/' + item.siteTime}}</label>
            </div>
          </template>
        </el-table-column>
